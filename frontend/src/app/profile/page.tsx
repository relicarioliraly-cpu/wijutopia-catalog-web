  ...
  const handleLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (payload.user.email !== 'admin@wijutopia.com' || payload.user.role !== 'empleado') {
        setMessage('Acceso denegado: este profile requiere el rol Empleado con admin@wijutopia.com.');
        return;
      }
      ...
      setMessage('Profile administrativo conectado.');
    } catch (error) {
  ...
  if (!isAdmin) {
    return (
      <main className="mx-auto max-w-2xl px-4 py-16">
        <AuthProfileCard
          adminOnly
          title="Profile de empleado"
          description="El panel antes llamado Admin ahora se llama Profile. Inicia sesión o registra una cuenta; para ver inventario y métricas se requiere admin@wijutopia.com con rol Empleado."
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-8 px-4 py-10">
      <section className="rounded-[2rem] border border-wiju-borderLight bg-wiju-cardLight p-8 shadow-card dark:border-wiju-borderDark dark:bg-wiju-cardDark">
        <p className="text-sm font-black uppercase tracking-[0.3em] text-wiju-crimson dark:text-wiju-gold">Profile administrativo</p>
        <h1 className="mt-3 text-4xl font-black">Profile: inventario y analítica implícita</h1>
